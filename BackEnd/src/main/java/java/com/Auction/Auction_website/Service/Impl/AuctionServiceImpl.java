...
public AuctionServiceImpl(AuctionState state,Player_Repo repo,Bid_Repo bid_repo,Team_Repo team_repo){
    this.state=state;
...
return state.getCurrentPlayerId()!=null;
}

    @Override
    public void finaliseBid(Long playerId) {
        Player player = player_repo.findById(playerId)
                .orElseThrow(() -> new RuntimeException("Player not found"));
        if(player.getStatus()==AuctionStatus.SOLD)
            return;
        Bid HighestBid=bid_repo.findTopByPlayerOrderByAmountDesc(player);
        if(HighestBid==null){
            System.out.println("No Bid for"+player.getName());
            return;
        }
        Team team = HighestBid.getTeam();
        if (team.getBudget() < HighestBid.getAmount()) {
            throw new RuntimeException("Team cannot afford winning bid");
        }
        // Mark SOLD
        player.setStatus(AuctionStatus.SOLD);
        player.setTeam(team);
        player.setSoldPrice(HighestBid.getAmount());
        player_repo.save(player);

        // Deduct budget
        team.setBudget(team.getBudget() - HighestBid.getAmount());
        team_repo.save(team);
